...
def main():
    ...
    os.system("mkdir -p ../07.permutation_test_variants")
    print("### Performing burden test (permutation test) with Permutations:{0}  Processors:{1}".format(args.numPermutation, args.process)) 
    cmd_perm = "cwas permutation_test -i ../06.category/all.sorted.categorization_result.zarr -o_dir ../07.permutation_test_variants -s ../SAMPLE_LIST.txt -a ../ADJUST_FACTOR.txt -n {0} -p {1} -b".format(args.numPermutation, args.process)
    os.system(cmd_perm)
